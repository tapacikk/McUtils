...
!***
   subroutine init_ttm2f_params(Nw)
!   use ttm2f_mod
   implicit none
...
!   ...........   gammaM
   ...
   end subroutine init_ttm2f_params

   subroutine ttm2f(Nw, RR, dRR, En, imodel)
      integer, intent(in) :: Nw, imodel
      ...
      call ttm2f_inner(Nw,RR,dRR,En, &
              Natsq, Natsd, Nats, fO, lO, fH, lH, fM, lM, fO3, lO3, fH3, lH3, fM3, lM3, imodel)
   end subroutine ttm2f

   subroutine ttm2f_inner(Nw,RR,dRR,En, &
           Natsq, Natsd, Nats, fO, lO, fH, lH, fM, lM, fO3, lO3, fH3, lH3, fM3, lM3, imodel)
...
!   use ttm2f_mod
   implicit none
   integer, intent(in) :: Nw
   integer, intent(in) :: fO, lO, fH, lH, fM, lM, fO3, lO3, fH3, lH3, fM3, lM3
   integer, intent(in) :: Nats, Natsd, Natsq,imodel
   double precision, dimension(3, 3*Nw), intent(in)  ::  RR
   double precision, dimension(3, 3*Nw), intent(out) :: dRR
   double precision, intent(out) :: En
   !
   ...
   ! parameters that _were_ in init_ttm2f but which I had to move out for thread safety
   double precision, dimension(fO:lM) :: charge
   double precision, dimension(3, fM:lM) :: RM
   double precision, dimension(3, fM:lM) :: dRM
   double precision, dimension(3*Natsd) :: dip, pr_dip
   double precision, dimension(fH:lM) :: phi
   double precision, dimension(fO3:lM3) ::Efq
   double precision, dimension(fO3:lH3) ::Efd
   double precision, dimension(fO3:lH3, fO3:lH3) ::DDT
