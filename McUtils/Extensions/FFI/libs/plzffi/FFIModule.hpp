...
//        typedef T (*Func)(const FFIParameters&);
   ...
   public:
       ...
       std::vector<T> call(FFIParameters &params, const std::string &var) {
           ...
           size_t block_size; // precompute the block size for taking chunks of data
           if (shape.size() > 1) {
...
//                    py_printf("....wat %lu\n", args.size());
   ...
   FFIModule ffi_from_capsule(PyObject* captup) { // TODO: make this cached against the stored cap_obj...
       ...
       if (debug::debug_print(DebugLevel::Excessive)) py_printf("Got FFIModule spec \"%s\"\n", capsule_obj.repr().c_str());
       ...
       if (debug::debug_print(DebugLevel::Excessive)) py_printf("  pulling pointer with name \"%s\"\n", mod.ffi_module_attr().c_str());
